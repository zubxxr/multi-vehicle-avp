...
class VehicleState(Enum):
    ...
    NOT_WAITING_FOR_VEHICLES_IN_QUEUE = auto()   
    MOVING_UP_IN_QUEUE = auto()                   
    DROPPING_OFF_OWNER_AFTER_WAITING_IN_QUEUE = auto()  
    WAITING_TO_MOVE_UP_IN_QUEUE = auto()
    ...
    SENDING_PARKING_RESERVATION_REQUEST = auto()                    
    ...
    DRIVING_TO_DROPOFF_FOR_RETRIEVAL = auto()            
    PICK_UP_OWNER = auto()    
    LEAVING_AREA = auto()    
    AVP_COMPLETE = auto()                          
...
def main(args=None):
    ...
    while rclpy.ok():
        ...
        if current_state == VehicleState.ARRIVED_AT_LOCATION:
            ...
            current_state = VehicleState.ON_STANDBY
        
        elif current_state == VehicleState.ON_STANDBY:
            ...
            if front_car_moved:
                ...
                current_state = VehicleState.NOT_WAITING_FOR_VEHICLES_IN_QUEUE
        ...
        elif current_state == VehicleState.NOT_WAITING_FOR_VEHICLES_IN_QUEUE:
            avp_command_listener.publish_vehicle_status("Vehicle(s) ahead are still stationary. Proceeding with drop-off.")
            current_state = VehicleState.DROPPING_OFF_OWNER_AFTER_WAITING_IN_QUEUE

        elif current_state == VehicleState.DROPPING_OFF_OWNER_AFTER_WAITING_IN_QUEUE:
            time.sleep(2)
            ...
            current_state = VehicleState.WAITING_TO_MOVE_UP_IN_QUEUE
            
        elif current_state == VehicleState.WAITING_TO_MOVE_UP_IN_QUEUE:
            wait_until_route_complete(route_state_subscriber)
            ...
            if parking_spots:
                ...
                if first_spot_in_queue != chosen_parking_spot:
                    ...
                    current_state = VehicleState.SENDING_PARKING_RESERVATION_REQUEST

            else:
                avp_command_listener.get_logger().warn("[WARN] No available parking spots received from the subscriber.")

        elif current_state == VehicleState.SENDING_PARKING_RESERVATION_REQUEST:
            ...
            current_state = VehicleState.DRIVING_TO_DROPOFF_FOR_RETRIEVAL

        elif current_state == VehicleState.DRIVING_TO_DROPOFF_FOR_RETRIEVAL:
            run_ros2_command(retrieve_vehicle_goal_pose)
            run_ros2_command(engage_autonomous_mode)
            route_state_subscriber.state = -1
            wait_until_route_complete(route_state_subscriber)
            current_state = VehicleState.PICK_UP_OWNER

        elif current_state == VehicleState.PICK_UP_OWNER:
